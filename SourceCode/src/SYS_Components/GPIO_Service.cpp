...
void Backlight_Init()
{
    // 设置背光PWM引脚 - 1000Hz PWM
    gpio_set_function(TFT_BACKLIGHT, GPIO_FUNC_PWM);
    uint slice_num = pwm_gpio_to_slice_num(TFT_BACKLIGHT);
    uint channel = pwm_gpio_to_channel(TFT_BACKLIGHT);
    ...
    const uint32_t delay_ms = 10; // 每步之间的延时（毫秒）
...
void Soldering_GPIO_Init()
{
    ...
    pinMode(Soldering_SLEEP, INPUT_PULLUP);
...
{
    // 读取焊台睡眠状态
    return !digitalRead(Soldering_SLEEP);
}
...
{
    pinMode(Heatgun_SLEEP, INPUT_PULLUP);
