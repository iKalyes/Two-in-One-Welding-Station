...
#ifndef _RAK14014_FT6336U_H__
#define _RAK14014_FT6336U_H__
...
{
public: 
  // Configure touch screen dimensions and pins (call before begin())
  void setTouchConfig(uint16_t width, uint16_t height, 
                      uint8_t intPin, uint8_t sdaPin, uint8_t sclPin, uint8_t rstPin);

  bool begin(TwoWire &wirePort = Wire, uint8_t i2cAddr = I2C_ADDR_FT6336U);
  bool available(void);
...
private:  
  ...
  uint8_t _deviceAddress = I2C_ADDR_FT6336U;

  // Touch screen configuration
  uint16_t _touchWidth;
  uint16_t _touchHeight;
  uint8_t _intPin;
  uint8_t _sdaPin;
  uint8_t _sclPin;
  uint8_t _rstPin;
