...
void FT6336U::setTouchConfig(uint16_t width, uint16_t height, 
...
bool FT6336U::begin(TwoWire &wirePort, uint8_t i2cAddr)
{
  ...
  delay(10);
  _i2cPort = &wirePort;
  _i2cPort->setSDA(_sdaPin);
  _i2cPort->setSCL(_sclPin);
  _i2cPort->begin(); 
  if(read_device_type() == 0x02)
  {
    writeByte(0x02, 0x01);
    attachInterrupt(digitalPinToInterrupt(_intPin), tpIntHandle, FALLING);
    return true;
  }
  return false;
}
...
FT6336U_TouchPointType FT6336U::scan(void){
    ...
    if(touchPoint.touch_count == 0) {
        ...
        touchPoint.tp[id1].x = _touchWidth - read_touch1_y(); 
